  ...
  <div class="space-y-5">
    ...
    <div class="stat-card">
      ...
      <div v-if="result" class="text-xs rounded-xl p-3" :class="result.ok && result.submitted > 0 ? 'bg-emerald-50 text-emerald-700' : result.ok && result.submitted === 0 ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'">
        <div v-if="result.ok && result.submitted > 0">
          ✓ Submitted {{ result.submitted }} pages to IndexNow
          <div v-if="result.urls?.length" class="mt-2 text-emerald-600 max-h-32 overflow-auto">
            <div v-for="url in result.urls.slice(0, 10)" :key="url" class="truncate">• {{ url }}</div>
            <div v-if="result.urls.length > 10" class="text-emerald-500">… and {{ result.urls.length - 10 }} more</div>
          </div>
        </div>
        <div v-else-if="result.ok && result.submitted === 0">
          No pages found. Go to the llms.txt tab → click "Generate" first, then try again.
        </div>
