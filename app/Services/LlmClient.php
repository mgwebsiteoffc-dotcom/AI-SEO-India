...
class LlmClient
{
    ...
    public function chat(string $system, string $user, bool $json = false): ?string
    {
        ...
        // Try each provider in order: OpenRouter → OpenAI → Gemini
        $providers = [];
        if (config('services.openrouter.key')) $providers[] = 'openrouter';
        if (config('services.openai.key')) $providers[] = 'openai';
        if (config('services.gemini.key')) $providers[] = 'gemini';

        foreach ($providers as $provider) {
            try {
                $result = match ($provider) {
                    'openrouter' => $this->openrouter($system, $user, $json),
                    'openai' => $this->openai($system, $user, $json),
                    'gemini' => $this->gemini($system, $user, $json),
                };
                ...
                Log::warning("LLM {$provider} returned null, trying next provider...");
            } catch (\Throwable $e) {
                Log::warning("LLM {$provider} failed: " . $e->getMessage() . ", trying next...");
