...
class SmartBlogger
{
    ...
    public function catalogProducts(Store $store, int $limit): array
    {
        $products = [];

        // Check if we can make Shopify API calls
        if (!ShopifyService::hasValidToken($store)) {
            // No token — use llms entries as fallback
