...
class AdminController extends Controller
{
    ...
    public function updatePlan(Request $request, Store $store)
    {
        ...
        // Use BillingService::activate() to create proper billing records
    ...
    /**
     * POST /admin/stores/{store}/toggle-custom — toggle custom distribution flag
     */
    public function toggleCustom(Store $store)
    {
        $store->update(['is_custom' => !$store->is_custom]);
        $status = $store->is_custom ? 'custom distribution (admin-managed billing)' : 'Shopify billing';
        return back()->with('status', "{$store->shop} set to {$status}.");
    }
