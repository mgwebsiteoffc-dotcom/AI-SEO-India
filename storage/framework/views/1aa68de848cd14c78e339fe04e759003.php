...
<body class="bg-slate-950 text-slate-100">
    <header class="border-b border-white/10 bg-slate-950/80 backdrop-blur sticky top-0 z-40">
        <div class="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <a href="<?php echo e(route('app', ['demo' => 1])); ?>" class="text-xs font-semibold text-slate-300 hover:text-white">Live demo →</a>
        </div>
    </header>
        ...
        <div class="max-w-3xl mx-auto px-4">
                ...
                <div class="flex items-center justify-between mb-6">
                    ...
                    <div class="text-right">
                        <div class="text-4xl font-extrabold grad-text" style="background:linear-gradient(90deg,#60a5fa,#38bdf8);-webkit-background-clip:text;background-clip:text;color:transparent;">
                            <?php echo e($demoScore['total'] ?? 74); ?><span class="text-base text-slate-500">/100</span>
                        </div>
                        <div class="text-xs text-slate-500">Grade <?php echo e($demoScore['grade'] ?? 'B'); ?></div>
                    </div>
                </div>
                <?php ($cats = $demoScore['categories'] ?? ['crawlability' => 92, 'schema' => 55, 'content' => 78, 'brand' => 85, 'speed' => 100]); ?>
                <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                    <?php $__currentLoopData = $cats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="text-center">
                        <div class="text-[11px] text-slate-500 capitalize"><?php echo e($k); ?></div>
                        <div class="text-xl font-bold <?php echo e($v >= 80 ? 'text-emerald-400' : ($v >= 60 ? 'text-amber-400' : 'text-red-400')); ?>"><?php echo e($v); ?></div>
                    </div>
            ...
            <div class="rounded-3xl bg-slate-900 border border-brand-500/40 p-6 md:p-8 mt-6">
                ...
                <?php if(session('status')): ?>
                    <div class="rounded-xl bg-emerald-500/15 border border-emerald-500/40 text-emerald-300 text-sm p-4 mb-4"><?php echo e(session('status')); ?></div>
                <?php endif; ?>
                <form method="POST" action="<?php echo e(route('lead')); ?>" class="space-y-3">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="source" value="scorecard">
                    <input type="email" name="email" required placeholder="you@brand.com" class="input !bg-slate-950 !border-white/15 !text-white" value="<?php echo e(old('email')); ?>">
                    <input type="text" name="brand" placeholder="Brand name (optional)" class="input !bg-slate-950 !border-white/15 !text-white" value="<?php echo e(old('brand')); ?>">
                    <input type="text" name="shop_url" placeholder="your-brand.myshopify.com or yourbrand.in (optional)" class="input !bg-slate-950 !border-white/15 !text-white" value="<?php echo e(old('shop_url')); ?>">
    ...
    <footer class="border-t border-white/10 py-8 text-center text-xs text-slate-500">
        <a href="<?php echo e(route('home')); ?>" class="hover:text-slate-300">← Back to home</a> · <a href="<?php echo e(route('pricing')); ?>" class="hover:text-slate-300">Pricing</a>
    </footer>
</body>
</html>
<?php /**PATH C:\laragon\www\ai-visibility-source\resources\views/marketing/scorecard.blade.php ENDPATH**/ ?>